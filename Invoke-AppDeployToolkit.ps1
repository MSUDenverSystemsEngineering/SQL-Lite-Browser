...
$adtSession = @{
    # App variables.
    AppVendor = 'Mauricio Piacentini'
    AppName = 'DB Browser for SQLite'
    AppVersion = '3.12.2'
    AppArch = 'x64'
    AppLang = 'EN'
    AppRevision = '01'
...
function Install-ADTDeployment
{
    ...
    Show-ADTInstallationWelcome -CloseProcesses 'DB Browser for SQLite' -CheckDiskSpace -PersistPrompt
    ...
    ForEach($installedApplication in $applicationList) {
        $installedApplicationList = Get-ADTApplication -Name $installedApplication
        ForEach($application in $installedApplicationList) {
            $application
            if($application.UninstallString) {
                Write-ADTLogEntry -Message "Uninstall string: $($application.UninstallString)" -Source 'Pre-Installation' -LogType 'CMTrace'
                Write-ADTLogEntry -Message "Uninstall subkey: $($application.UninstallSubkey)" -Source 'Pre-Installation' -LogType 'CMTrace'
                ...
                if($application.UninstallString.contains("MsiExec.exe") -and ($application.UninstallSubkey)) {
                    Write-ADTLogEntry -Message "Attempting to run uninstaller..." -Source 'Pre-Installation' -LogType 'CMTrace'
                    $exitCode = Start-ADTProcess -FilePath "MsiExec.exe" -ArgumentList "/x$($application.UninstallSubkey) /q" -WindowStyle "Hidden"-IgnoreExitCodes '1603' -PassThru
    ...
    $exitCode = Start-ADTMsiProcess -Action 'Install' -FilePath "$($adtSession.DirFiles)\DB.Browser.for.SQLite-v3.13.1-win64.msi" -ArgumentList 'REBOOT=ReallySuppress /QN' -PassThru
    If (($exitCode.ExitCode -ne "0") -and ($mainExitCode -ne "3010")) { $mainExitCode = $exitCode.ExitCode }
...
function Uninstall-ADTDeployment
{
    ...
    Show-ADTInstallationWelcome -CloseProcesses 'DB Browser for SQLite' -PersistPrompt
    ...
    Write-ADTLogEntry -Message "Attempting to run uninstaller..." -Source 'Uninstallation' -LogType 'CMTrace'
    $exitCode = Start-ADTMsiProcess -Action 'Uninstall' -ProductCode "{541AE182-7C1D-426C-8155-4867303B75A4}" -ArgumentList '/qn' -PassThru
    If (($exitCode.ExitCode -ne "0") -and ($mainExitCode -ne "3010")) { $mainExitCode = $exitCode.ExitCode }
